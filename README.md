...
- **Hover translate** — translate the word/block under cursor in normal mode
...
- **Any language** — works with Chinese, Korean, French, Arabic, etc. (auto-detect)
- **Configurable target** — translate to English, Indonesian, Chinese, or any language
- **LSP fallthrough** — on English/code text, falls through to LSP hover as normal
...
```lua
{
  ...
  opts = {
    target_lang = 'en',     -- target language (e.g. 'en', 'id', 'zh', 'ko')
    backend = 'google',     -- 'google' or 'claude'
    keymap = {
...
|----------|-------------|
| `require('nvim_translate').translate_word()` | Translate word/block under cursor |
...
If the text under cursor is pure ASCII, `translate_word()` falls through to `vim.lsp.buf.hover()` — so sharing a key with LSP hover works naturally.
...
3. Detects if the text is Japanese (for better source-language hint)
